function selectShow(selectNative, selectCustom, selectCustomSelected, selectCustomOptions, selectCustomOptionsList, selectCustomOption, optionsCount) {
...
function quantity(quantityBtn) {
  quantityBtn.forEach(item => {
    item.addEventListener("click", () => {
      let value = parseInt(item.parentNode.querySelector("input").value);
      if (!isNaN(value) && value >= 0) {
        if (item.classList.contains("quantity__btn-plus")) {
          value++
        } else {
          value = value - 1;
          if (value < 1) {
